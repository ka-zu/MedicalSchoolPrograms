...
class MLP(Chain):
    def __init__(self):#層の様子を定義
        ...
        super(MLP, self).__init__(
            l1=L.Linear(51, 200),#68個のベクトルで69次元
...
for f in testCSV:
    ...
    #Normの行を削除する
    normIndexes = testDataDF.columns  # カラム名を取得
    normIndexes = [s for s in normIndexes if 'Norm' in s]  # Normとつく行を抽出
    testDataDF = testDataDF.drop(normIndexes, axis=1)
    ...
    #識別したいデータ
    testData = testDataDF.iloc[:,1:52].values
    predictData = np.empty((0,51), float)
    ...
    #ロード
    serializers.load_hdf5('myMLP_noNorm.model',model)
    ...
    #csv出力
    resultDF.to_csv("./result//"+filename+"_result_noNorm.csv", sep=",", index=False, header=True, mode='w')
