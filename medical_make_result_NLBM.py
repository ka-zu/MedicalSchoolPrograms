...
for f in testCSV:
    ...
    #識別したいデータ
    testData = testDataDF.iloc[:,1:45].values
    predictData = np.empty((0,44), float)
    ...
    #ロード
    serializers.load_hdf5('NoLowerBody.model',model)
    ...
    #csv出力
    resultDF.to_csv("./result//"+filename+"_result_NLMB.csv", sep=",", index=False, header=True, mode='w')
