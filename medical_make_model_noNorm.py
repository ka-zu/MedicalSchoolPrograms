...
class MLP(Chain):
    def __init__(self):#層の様子を定義
        ...
        super(MLP, self).__init__(
            l1=L.Linear(51, 200),#52個のベクトルで52次元
...
for (f,l) in zip(csvfiles,linefiles):
    ...
    #ベクトルの長さの部分を削除
    normIndexes = tmpDataDF.columns#カラム名を取得
    normIndexes = [s for s in normIndexes if 'Norm' in s]#Normとつく行を抽出
    tmpDataDF = tmpDataDF.drop(normIndexes,axis=1)
...
x_train = dataDF.iloc[:,1:52].values
...
#modelをmyMlPの名前で保存
serializers.save_hdf5('myMLP_noNorm.model',model)
